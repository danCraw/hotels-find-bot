...
@dp.message_handler(
    lambda msg: not msg.text.startswith("/"), state=FSMClient.verifying_codes
)
    ...
    async with state.proxy() as data:
        ...
        if data["attempts_left"] < 0:
            await message.answer(
                "🚫 Превышено количество попыток. Начните процесс заново."
            )
            await state.finish()
            return
        ...
        try:
            input_data = {
                ...
                for k, v in (line.split(":", 1) for line in message.text.split("\n"))
            }
            ...
            if email_match and phone_match:
                await message.answer(
                    "✅ Коды подтверждены! Переходим к созданию заказа "
                )
                await state.finish()
            else:
...
@dp.message_handler(
    lambda message: message.text == "Отправить коды повторно",
    state=FSMClient.verifying_codes,
)
async def resend_verification_codes(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        if current_time - last_resend < 30:
            ...
            await message.answer(
                f"⏳ Повторная отправка будет доступна через {remaining} секунд"
            )
            return
