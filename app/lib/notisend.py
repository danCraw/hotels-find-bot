...
class SMS:
    ...
    def sendSMS(self, recipients, message, sender="", run_at="", test=0):  # шлём SMS
        rqData = {"recipients": recipients, "message": message, "test": test}
        if sender != "":
    ...
    def messagePrice(self, recipients, message):  # узнаём цену сообщений
        rqData = {"recipients": recipients, "message": message, "sender": self.sender}
    ...
    def create(
        ...
        rqData = {"include": include, "message": message}
        if sender != "":
            rqData["sender"] = sender
