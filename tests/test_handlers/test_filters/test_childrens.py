...
@pytest.mark.asyncio
@pytest.mark.parametrize("text, expected_reply, expected_next_state", [
    ...
    ("2", "Введите возраст для каждого из 2 детей через запятую", FSMClient.waiting_for_children_age),
])
...
@pytest.mark.asyncio
@pytest.mark.parametrize("children, input_text, expected_reply, should_pass", [
...
async def test_process_children_age_input(children, input_text, expected_reply, should_pass):
    message = AsyncMock(spec=Message)
    ...
    if should_pass:
        state.update_data.assert_called_once_with(children_age=[int(age) for age in input_text.split(",")])
        FSMClient.days_of_stay.set.assert_called_once()
    else:
        state.update_data.assert_not_called()
